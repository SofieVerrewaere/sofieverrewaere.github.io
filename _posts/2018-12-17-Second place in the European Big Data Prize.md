...
Zero values occur frequently in the training data (about one in three data points). Unlike the interpolated values, these represent real target values and hence require a special treatment. The huge amount of zero values is captured by defining two types of targets, targets for regular (non-zero) values and the probability of zero values. The combined forecast is the probability of a non-zero value times the forecasted non-zero value which corresponds to the regression target in expectation.
